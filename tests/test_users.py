...
from provisioner.provision import Users, User, load_users_config, UsersConfig, UsersDiff, users_in_sudoer_file, \
    ResourceState, ResourceMissing, ResourceOutdated, ResourcePresent
...
def test_users_state_1() -> None:
    ...
    assert Users(id=TEST_USER_ID).state(
        Users(id=TEST_USER_ID, users=frozenset({generate_test_user("user1")}))
            ...
            users_to_delete=frozenset({generate_test_user("user1")}))
...
def test_users_state_2() -> None:
    ...
    assert Users(
        ...
        users_final=us,
        sudoers_final=frozenset([u.name for u in us])
    )
...
async def test_load_users_config_0() -> None:
    with mock_commands(
        pre_users_config=(uc := UsersConfig(
            users=frozenset(
                {
                    generate_test_user("user1"),
                    generate_test_user("user2"),
